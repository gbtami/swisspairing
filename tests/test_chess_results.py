...
def _budapest_group_b_manifest_path() -> Path:
...
def _graz_round_entry(round_number: int) -> dict[str, Any]:
    return _chess_results_round_entry("international_chessopen_graz_2026_a", round_number)


def _graz_states_for_round(round_number: int) -> tuple[PlayerState, ...]:
...
def test_checked_in_budapest_group_b_manifest_references_existing_trfs() -> None:
...
def test_aeroflot_round_5_exact_pairs_full_round() -> None:
    manifest_path = _aeroflot_manifest_path()
...
def test_graz_round_4_score_5_bracket_keeps_recent_downfloater_in_bracket() -> None:
    manifest_path = _graz_manifest_path()
    round_entry = _graz_round_entry(4)
    trf = TrfParser.parse(manifest_path.parent / cast(str, round_entry["trf"]))
    initial_color = build_trf_initial_color(trf)
    scoregroups = _group_residents_by_score(_graz_states_for_round(4))

    mdp = next(player for player in scoregroups[4] if player.pairing_no == 273)
    bracket_players = tuple(
        sorted(
            (mdp, *scoregroups[5]),
            key=lambda player: (-player.score, player.pairing_no),
    ...
    assert result.unpaired_ids == ("259",)
    assert frozenset({"216", "275"}) in paired_ids
...
def test_budapest_group_b_round_7_exact_pairs_full_round() -> None:
    manifest_path = _budapest_group_b_manifest_path()
    round_entry = _budapest_group_b_round_entry(7)
    trf = TrfParser.parse(manifest_path.parent / cast(str, round_entry["trf"]))
